#!/usr/bin/env python3
"""
EPIC-KITCHENS-100 - Optimized Full Dataset Training
====================================================
With gradient accumulation, more frames, and resume support.
"""
...
def train_epoch(model, loader, optimizer, scaler, verb_criterion, noun_criterion,
                device, cutmix_alpha=0.0, accumulation_steps=1):
    model.train()
    total_loss = 0
    verb_correct = 0
    noun_correct = 0
    ...
    for batch_idx, (frames, verb_labels, noun_labels) in enumerate(pbar):
        ...
        with autocast('cuda'):
            ...
            loss = (verb_loss + noun_loss) / accumulation_steps
...
def main():
    ...
    # Datasets
    print(f"\nLoading datasets with {args.num_frames} frames per action...")
    ...
    print(f"\nBatch size: {args.batch_size} x {args.accumulation_steps} accumulation = {effective_batch} effective")
    ...
    if args.resume:
        ...
        best_action_acc = checkpoint.get('best_action_acc', 0)
        print(f"  Resumed at epoch {start_epoch}, best acc: {best_action_acc*100:.2f}%")
    ...
    print(f"Frames: {args.num_frames} | Batch: {effective_batch} | LR: {args.lr}")
    print(f"{'='*60}\n")
